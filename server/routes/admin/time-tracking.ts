...
import { insertVacationAllowanceSchema } from "@shared/schema";
...
router.get("/employee-appointments", asyncHandler("Termine konnten nicht geladen werden", async (req: Request, res: Response) => {
  ...
  const servicesByAppt = await timeTrackingStorage.getAppointmentServiceDetailsByAppointmentIds(apptIds);
