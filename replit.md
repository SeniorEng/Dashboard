...
- **Statistics Page (`/admin/statistics`)**: Redesigned from 7 tabs to 4 consolidated tabs:
  - **Cockpit** (default): 3 KPI-Karten (DB-Marge, Auslastung, §45b Budget) mit Ampel-Farbcoding und Vormonats-Trend (▲▼→). Handlungsbedarf-Bereich mit priorisierten Alerts (rot/gelb/grün). Pflegegradverteilung + Monats-Stunden-Chart darunter. Endpoints: `GET /api/statistics/overview` (inkl. `cockpit` Objekt), `GET /api/statistics/alerts`.
