...
- **Dienstleistungskatalog (Service Catalog)**: Central management of services with code, name, unit type, standard price, VAT rate. Customer-specific price overrides are supported. Dynamic service selection for appointments, storing services in a junction table (`appointment_services`). Legacy appointment columns (`hauswirtschaftDauer`, `alltagsbegleitungDauer`, `erstberatungDauer` + Actual variants) are deprecated but still populated for backward compatibility. New code should use junction table data and the `*FromServices()` domain functions. Documentation flow now updates both legacy columns AND junction table `actualDurationMinutes`/`details`. Time-conflict and auto-break calculations use `durationPromised` instead of legacy column sums. Frontend components (appointment cards, detail pages, service records, time tracking) now read from junction table or `durationPromised`/`serviceType` instead of legacy columns. PATCH and document endpoints use database transactions for atomic junction table updates. ServiceHoursSummary aggregation uses junction table data. Remaining legacy refs (240) are schema/type definitions, backward-compat writes, SELECT queries, and deprecated function bodies — all auto-resolve when columns are dropped.
