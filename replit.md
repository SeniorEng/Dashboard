...
- **Customer Documents (Bestandskunde)**: Document-type-centric UI in `customer-documents-section.tsx`. Shows all doc types for `bestandskunde`/`beide` context. Each row displays status (vorhanden/fehlt/abgelaufen), uploaded docs, and generated digital docs grouped by type. Actions: "Digital erstellen" (if template exists) + "Hochladen" (always). Orphaned generated docs shown separately.
