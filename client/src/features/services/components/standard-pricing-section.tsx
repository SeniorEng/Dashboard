...
interface StandardPricingSectionProps {
  services: ServiceWithPots[] | undefined;
  isLoading: boolean;
}

export function StandardPricingSection({ services, isLoading }: StandardPricingSectionProps) {
  ...
  const { data: standardPrices, isLoading: loadingPrices } = useQuery<StandardPrice[]>({
    ...
    queryFn: async () => {
      const result = await api.get<StandardPrice[]>("/services/standard-prices");
  ...
  const { data: futurePrices } = useQuery<StandardPrice[]>({
    ...
    queryFn: async () => {
      const result = await api.get<StandardPrice[]>("/services/standard-prices/future");
      return unwrapResult(result);
    },
    staleTime: 30000,
  });
  ...
  const { data: allPrices } = useQuery<StandardPrice[]>({
    ...
    queryFn: async () => {
      const result = await api.get<StandardPrice[]>("/services/standard-prices/all");
      return unwrapResult(result);
    },
    staleTime: 30000,
    enabled: showHistory,
  ...
  const saveMutation = useMutation({
    mutationFn: async ({
      ...
      const result = await api.post("/services/standard-prices", body);
      return unwrapResult(result);
    },
    onSuccess: (_data, variables) => {
      ...
      toast({ title: variables.confirmReplace ? "Bestehender Standardpreis ersetzt" : "Standardpreis gespeichert" });
    },
  ...
  const priceMap = new Map<number, StandardPrice>();
  standardPrices?.forEach(sp => priceMap.set(sp.serviceId, sp));

  const futureByService = new Map<number, StandardPrice[]>();
  ...
  if (activeServices.length === 0) {
    return <p className="text-sm text-gray-500 py-4 text-center" data-testid="text-no-prices">Keine abrechenbaren Dienstleistungen im Katalog</p>;
  }
  ...
  function startEdit(serviceId: number, currentPriceCents: number) {
    ...
    setEditPrice(formatEuroDE(currentPriceCents, { withCurrency: false }));
    setEditValidFrom("");
  }
  ...
  return (
          ...
          <AlertDialogDescription>
            {pendingReplace ? (
              <>
                ...
                bereits ein aktiver Standardpreis ab dem{" "}
    ...
    <div className="space-y-1" data-testid="standard-pricing-section">
      {hasFuturePrices && (
      ...
      {activeServices.map((service) => {
        ...
        const standardPrice = priceMap.get(service.id);
        const effectivePrice = standardPrice ? standardPrice.priceCents : service.defaultPriceCents;
        const isEditing = editingServiceId === service.id;
        const isOverridden = !!standardPrice;
        const serviceFuturePrices = futureByService.get(service.id) || [];

        return (
          <div key={service.id} data-testid={`pricing-row-${service.id}`}>
            <div className={`grid grid-cols-[1fr_auto_auto] gap-x-3 items-center px-2 py-2 rounded-lg hover:bg-gray-50 ${isOverridden ? 'bg-teal-50/50' : ''}`}>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium truncate">{service.name}</span>
                  {isOverridden && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-teal-100 text-teal-700 font-medium">aktiv ab {formatDateDisplay(standardPrice!.validFrom)}</span>
                  )}
                </div>
                {isOverridden && (
                  <div className="text-[11px] text-gray-500 mt-0.5">
                    Katalog-Default: {formatCurrency(service.defaultPriceCents)}{unitLabel}
                  </div>
                )}
              ...
              <div className="text-right w-24">
                {isEditing ? (
                  ...
                  <>
                    <span className={`text-sm font-semibold ${isOverridden ? 'text-teal-700' : ''}`}>{formatCurrency(effectivePrice)}</span>
      ...
      <div className="px-2 pt-2 space-y-2">
        <p className="text-[11px] text-gray-500">
          Preise zzgl. MwSt. Klicken Sie auf den Stift, um einen firmenweiten Standardpreis zu setzen.
          {" "}Über das Datumsfeld legen Sie einen „gültig ab"-Stichtag fest (frühestens heute).
          {" "}Kundenindividuelle Preise haben weiterhin Vorrang vor dem Standardpreis, der Standardpreis vor dem Katalog-Default.
          {" "}Eine Änderung legen Sie als neue Korrekturphase ab Datum an — bestehende Historieneinträge bleiben revisionssicher (GoBD) und werden nicht überschrieben.
        </p>
        <Button
          variant="ghost"
          size="sm"
                ...
                {allPrices.map(p => (
                  <tr key={p.id} className={`border-t ${!p.validTo ? 'bg-teal-50/50' : ''}`} data-testid={`history-row-${p.id}`}>
                    ...
                    <td className="px-2 py-1.5 text-right font-medium">
                      <span data-testid={`text-history-price-${p.id}`}>{formatCurrency(p.priceCents)}{UNIT_LABELS[p.unitType] || ""}</span>
                    </td>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm">
                <p>
                  Sie ändern einen firmenweiten Standardpreis mit einem Stichtag, der bereits
                  in abgerechneten Zeiträumen liegt. Diese Änderung könnte erneute
                  Rechnungserstellungen mit anderen Preisen abrechnen, als ursprünglich auf der
                  Rechnung standen.
                </p>
                ...
                <p className="text-xs text-gray-600">
                  Bestehende, gesiegelte Rechnungen bleiben unverändert. Bitte bestätigen Sie nur,
                  wenn Sie sicher sind. Die Aktion wird im Audit-Log dokumentiert.
                </p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
