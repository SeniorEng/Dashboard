...
QUELLE_SSLMODE="${QUELLE_SSLMODE:-require}"
...
quelle() ( export PGHOST="$QUELLE_HOST" PGPORT="$QUELLE_PORT" PGUSER="$QUELLE_USER" PGPASSWORD="$QUELLE_PW" \
           ...
           PGOPTIONS="-c default_transaction_read_only=on"; exec "$@" )
...
RO=$(quelle psql -X -At -c "SHOW default_transaction_read_only")
[[ "$RO" == "on" ]] || fail "Prod-Verbindung ist nicht read-only"
...
quelle pg_dump --snapshot="$SNAP" --schema=public --no-owner --no-privileges --no-comments -Fp \
...
msql postgres <<SQL
SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${MIRROR_DB}' AND pid <> pg_backend_pid();
DROP DATABASE IF EXISTS ${MIRROR_DB} WITH (FORCE);
ALTER DATABASE ${STAGING} RENAME TO ${MIRROR_DB};
DO \$\$ BEGIN
