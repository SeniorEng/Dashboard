 ...
 *
 * ── Und die Abweichung, die daran hängt ────────────────────────────────
 * `applyInitialBudget` lehnt Startwert + Übertrag nur ab, wenn der Übertrag
 * **> 0** ist (bewusste Abweichung von Alriks wörtlicher Vorgabe, im PR
 * begründet). `EO-5` sichert die abgelehnte Richtung; die ANGENOMMENE hatte
 * keinen Gegenpart. `NU-1` ist er.
 */
...
describe("§45b — 0-€-Übertrag neben einem Startwert", () => {
  ...
  it("NU-2 – ein Übertrag > 0 neben demselben Startwert wird ABGELEHNT", async () => {
    // Die Grenze der Abweichung: `> 0` ist der Unterschied, nicht „Übertrag
    // vorhanden". Ohne diesen Test wäre NU-1 auch erfüllt, wenn gar nichts
    // mehr abgelehnt wird.
    ...
    try {
      ...
      expect(res.status, "Startwert + echter Übertrag wurde nicht abgelehnt").toBe(400);
      expect(await zeilen(id), "trotz Ablehnung wurde geschrieben").toEqual([]);
    } finally {
