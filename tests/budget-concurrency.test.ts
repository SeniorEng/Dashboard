...
describe("BC-K4: Advisory-Lock serialisiert parallele Konsumbuchungen", () => {
  it("BC-K4.1 — 5 parallele Buchungen je 30 € bei 100 € Budget (acceptsPrivatePayment=false): max 100 € verbraucht, mind. 1 Ablehnung", async () => {
    ...
    // Frischer Mitarbeiter pro Test = leerer Kalender → keine Slot-Konflikte
    // mit Termin-Pollution aus früheren Test-Läufen.
    const employee = await createTestEmployee({ nachnamePrefix: "BC-K4" });
    ...
    // sodass der Lock-Test eine echte Ablehnung erzwingt. Direkter DB-Insert,
    // damit der Test unabhängig von Kalender-Pollution durch andere Suite-
    // Schritte wird (assignedEmployeeId ist ein frisch erstellter Mitarbeiter
  ...
  // Hinweis: Ein dedizierter Konkurrenztest für den Cascade-Pfad mit
  // hauswirtschaftCost=0 wurde bewusst nicht aufgenommen. Der Advisory-Lock
  // (`pg_advisory_xact_lock(hashtext('budget_consumption_'||customerId))`)
  // wird in `consumption-engine.ts:doWork` als allererste Anweisung gesetzt —
  // BEVOR sich die Pfade `hasUsage=true` (FIFO) und `hasUsage=false`
  // (Cascade/Cost=0) verzweigen. Damit serialisiert derselbe Lock beide
  // Pfade per Konstruktion. BC-K4.1 verifiziert das Locking funktional über
  // den FIFO-Pfad; eine Wiederholung über den Cascade-Pfad würde lediglich
  // dieselbe Code-Stelle erneut treffen, ohne zusätzliche Aussage.
});
