/**
 * Task #1024 — End-to-End-Verifikation des Phantom-Topf-Rechnungs-Reconcile
 * (`reconcilePhantomPotInvoices`, Task #1016).
 *
...
beforeAll(async () => {
  ...
  await db.insert(budgetTransactions).values({
    ...
    amountCents: REVERSED_45A_CENTS,
    appointmentId,
    reversedTransactionId: orig45a.id,
...
describe("reconcilePhantomPotInvoices — End-to-End (Task #1024)", () => {
  ...
  it("Scharflauf storniert die §45a-Rechnung GoBD-konform und lässt §45b unberührt", async () => {
    const ledgerBefore = await snapshotCustomerLedger();
    const REASON = "Phantom-Topf-Rechnung Egon-Uhlig (Task #1024 E2E-Verifikation)";
