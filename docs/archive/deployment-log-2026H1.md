# Deployment-Logbuch — Archiv (bis 2026-06)

> Archivierte Deployment-Log-Einträge. Aus dem Hot-Doc-Set ausgelagert (Task-Kontext-Kosten).
> Der aktuelle, gepflegte Live-Tail liegt in [`../deployment-log.md`](../deployment-log.md).
> Format pro Eintrag siehe `../pre-publish-backup-runbook.md`, §5. Nur-Lese-Historie, wird nicht mehr ergänzt.
...
### 2026-05-27 — Task #696: §45b-Budget zwei Bugfixes (Vorbereitung, kein Publish)
- **Bug 1 (Read-Pfad):** GET `/api/budget/:customerId/type-settings` lieferte während einer echten Settings-Transition (alte Zeile `validTo = heute`, neue Zeile `validFrom = heute+1`) die alte Zeile mit dem transienten `validTo = heute` zurück. Der Admin sah „Gültig bis = heute" wiederauftauchen, ein Clear+Save wurde zum No-Op (die neue offene Zeile hatte `validTo = null` bereits, der Equality-Check fand nichts zu ändern). Fix: neuer Helper `getLatestBudgetTypeSettings(customerId)` (preferences-storage.ts) liefert pro Topf die LATEST-INTENT-Zeile — offene Zeile (`validTo IS NULL`) schlägt schließende Zeile, Tie-Break über spätestes `validFrom` und höchste `id`. Route wechselt von `getBudgetTypeSettings` (= for-today) auf `getLatestBudgetTypeSettings`. Booking-Pfade (`consumption-engine`, `import-availability`, Auto-Allocation, Rebook) bleiben unverändert auf `getActiveBudgetTypeSettings(transactionDate)` — GoBD-Historisierungs-Semantik nicht angetastet.
- **Bug 2 (Calc-Pfad):** `calculateAllocated45b` lief bei Kunden mit `budget_preferences.budget_start_date` im Vorjahr UND einer existierenden Carryover-Allokation für das aktuelle Jahr über den Vorjahres-Monat (z.B. Dez 2025) hinweg und schlug 131 €/Monat auf, obwohl der Carryover dasselbe Restguthaben bereits abbildet → Doppelzählung. Prod-Fall „Schröder" (27.05.2026): „Gesamt zugewiesen" zeigte 1130,50 € statt 999,50 € (= 5×131 + 344,50). Fix: nach dem bestehenden Initial-Balance-Shift schiebt eine neue carryover-aware Bedingung `allocStart` auf den 1. Januar des spätesten gezählten Carryover-Zieljahrs vor (`countedCarryoverYears` respektiert den bestehenden `ibYears`-Filter aus Task #101, sodass durch IB blockierte Carryovers keinen Shift triggern und Vorjahres-Monate nicht unrechtmäßig verlorengehen).
