...
**EINE Nutzbarkeits-SSoT „ist Topf X am Datum Y nutzbar?" (Task #1838).** Die Frage, ob ein Budgettopf zu einem Stichtag als Ziel/Quelle nutzbar ist, wird an EINER Stelle beantwortet: der puren Funktion `resolvePotEligibilityAt({customer, settingsAtDate, configuredEverTypes, asOfDate})` in `shared/domain/budgets.ts`. Sie liefert je `budget_type` `eligible: true` oder `{eligible:false, reason}` (`out_of_window_configured` | `disabled` | `not_yet_valid` | `expired`). Sowohl die **Neu-Buchungs-Cascade** (`createCascadeConsumption` in `server/storage/budget/consumption-engine.ts`) als auch die **(Sammel-)Umbuchung** (`resolveRebookPotEligibility` in `server/storage/budget/rebook-storage.ts`, mappt die Gründe nur auf „Ziel-Topf …"-Meldungen) leiten ihre Topf-Kapazität-0-/Ablehnungs-Entscheidung aus dieser einen Funktion ab — keine zwei parallelen Ableitungen.
- **Entscheidung: Existenz-Gate in BEIDE Pfade gehoben (nicht fallengelassen).** Ein Topf, der IRGENDWANN konfiguriert wurde (`configuredEverTypes` via `readBudgetTypeSettings(..., {kind:"forEdit"})`), dessen Gültigkeitsfenster den Stichtag aber NICHT abdeckt, gilt als `out_of_window_configured` und ist NICHT nutzbar — er fällt NICHT still über den anspruchs-gegateten Default (`effectiveDefaultPots`, §45b für Pflegekassen-Kunden default-aktiv) zurück. Rationale: Hat ein Admin bewusst ein Gültigkeitsfenster gesetzt, darf sich der Topf außerhalb dieses Fensters nicht heimlich reaktivieren — weder beim Buchen noch beim Umbuchen.
- **Ersetzt** die frühere Asymmetrie: die Cascade nutzte inline `!enabled`/`validFrom`/`validTo`-Gates und fiel bei fehlender Stichtags-Zeile auf den Default zurück (Topf bleibt buchbar), während die Umbuchung bereits das Existenz-Gate (`forEdit`) anwandte und den Topf ablehnte. Nur ein „nie konfigurierter" Topf (keine Zeile in `configuredEverTypes`) erhält weiterhin den echten Default.
- **Regression:** `tests/billing/rebook-month.test.ts` → „Nutzbarkeits-SSoT: Buchung ≡ Umbuchung (Task #1838)" prüft, dass ein out-of-window-konfigurierter §45b-Topf MIT Guthaben in Cascade (Kapazität 0 → privater Überlauf) UND Umbuchungs-Preview (`eligible=false`) identisch als nicht nutzbar behandelt wird.
...
| Feld | Bedeutung |
|---|---|
| `budgetSetupRequired: boolean` | `true`, wenn der Kunde **keinen aktiven Budget-Topf** hat. Ermittelt über die Aktivierungs-SSoT `hasActiveBudgetPot` (`shared/domain/budgets.ts`) — Default-Töpfe gemerged mit den persistierten Overrides. Für Selbstzahler immer `false` (kein Anspruch). |
...
**Task #1828 hat diese Regel geändert — die frühere Fassung steht nicht mehr.**
Vorher: „pflegekassenberechtigt mit `pflegegrad >= 2` UND kein `budgets`-Block im
Payload". Seither entscheidet die Aktivierungs-SSoT, und das **Pflegegrad-Gate
ist ersatzlos entfallen** (§45b gilt gesetzlich ab PG 1).

Die praktische Folge ist wichtiger, als sie aussieht: **§45b ist für jeden
Nicht-Selbstzahler default-aktiv, auch ohne persistierte Zeile.** Ein frisch
angelegter Pflegekassen-Kunde hat damit immer einen aktiven Topf — auf dem
...
```jsonc
// PG4 Pflegekasse, Anlage ohne budgets-Block — §45b ist default-aktiv,
// es ist nichts einzurichten
{ "id": 8123, "name": "...", "billingType": "pflegekasse_gesetzlich",
  "budgetSetupRequired": false, "requiredBudgetTypes": [] }

// PG4 Pflegekasse, Anlage MIT budgets-Block (Wizard-Pfad)
{ "id": 8124, "...": "...", "budgetSetupRequired": false, "requiredBudgetTypes": [] }

// Selbstzahler — kein Anspruch auf gesetzliche Töpfe
{ "id": 8125, "...": "...", "budgetSetupRequired": false, "requiredBudgetTypes": [] }

// Replay (200) NACHDEM §45b per PUT type-settings abgeschaltet wurde:
// kein aktiver Topf mehr -> der Marker greift
{ "id": 8126, "idempotent": true, "budgetSetupRequired": true,
