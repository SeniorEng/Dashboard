...
| Conservation-Verifier (I13, Task #895) | `server/lib/budget-conservation.ts` (`computePotConservation`, `transactionType IN ('consumption','write_off')` minus `reversal`) | **Als Used gezählt** | Korrekt. Die No-Overdraw-Invariante prüft die **Topf-/Allocation-Sicht**: ein `write_off` hat den Topf entwertet und muss in den NettoKonsum eingehen, damit der Check „NettoKonsum ≤ Σ Allocated" nicht fälschlich Luft sieht, die längst abgeschrieben ist. Read-only; genutzt vom Migrations-Guard (Pre-/Post) und dem CLI-Verifier. | **Bestätigt korrekt** |
...
- **Gate I18 bestätigt:** Shadow-Read-Soak zeigt Δ0 auf §45a/§39 und nur die erwartete, akzeptierte virtuelle §45b-Divergenz (Legacy all-time vs. unified as-of + `manual_adjustment`). Keine Reparatur der Legacy-Mathematik — Auflösung erst mit §45b-Materialisierung (Phase 2). Begründung + IKS-Liste: [`docs/architecture/budget-verfahrensdokumentation.md`](architecture/budget-verfahrensdokumentation.md).
