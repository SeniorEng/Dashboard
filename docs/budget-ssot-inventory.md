...
| Stelle | Was | Wofür | Inputs | Drift-Hinweis |
|---|---|---|---|---|
| `getTotalCarryoverCents` | Summe **aktiver** Carryover-Allocations zum Stichtag | `computeCapSlot` (§45b-Window), Overview | `budget_allocations` | Konsumiert die rohen Cents — KEINE Berücksichtigung des bereits verbrauchten Carryover-Anteils (das macht `readBudget45bFifoBreakdown`) |
| ~~`getAvailableCarryoverCents`~~ | **ENTFERNT am 23.09.2026.** Die Spalte „Verbraucher“ behauptete eine „Detail-Anzeige Carryover-Tabelle“ — die gab es nicht. Die Funktion war nicht exportiert und hatte in ihrem einzigen erreichbaren Bereich keinen Aufrufer. Ihre Pro-Allocation-Rest-Mathematik führt `readBudget45bFifoBreakdown` inline. | — | — | Diese Zeile bleibt als Warnung stehen: eine Inventur, die einen Verbraucher nennt, wird als Beleg gelesen. |
...
`readBudget45bFifoBreakdown` rechnen sie je eigenständig — nur die erste ist
...
- `readBudget45bFifoBreakdown` (Pro-Allocation-Detail; die frühere Zweitfassung `getAvailableCarryoverCents` ist am 23.09.2026 als toter Code entfernt)
...
| Per-Allocation-Verbrauchsrechnung (Carryover) | `server/storage/budget/fifo-breakdown.ts` — seit dem 23.09.2026 die einzige Fassung; die Zweitfassung in `summary-queries.ts` war tot und ist entfernt | **Als Used gezählt** | Korrekt. Der pro Carryover-Allokation berechnete Rest darf den auto-`write_off` (Verfallsbuchung) als Verbrauch sehen, damit `Math.max(0, alloc - consumed)` nicht doppelt einen schon abgeschriebenen Rest zeigt. | **Bestätigt korrekt** |
...
| Consumption-Engine FIFO-Carryover | `server/storage/budget/consumption-engine.ts:163,208` | **Als Used gezählt** (in der Berechnung „wieviel ist von dieser Spezial-Allokation noch übrig") | Korrekt. Spiegelt dieselbe Allocations-pro-Stück-Logik wie `readBudget45bFifoBreakdown`. | **Bestätigt korrekt** |
...
| §45b FIFO-Aufschlüsselung (Task #1129) | `server/storage/budget/fifo-breakdown.ts` (`transactionType IN ('consumption','write_off')` für den Übertrags-Verbrauch pro Carryover-Allokation) | **Als Used gezählt** | Korrekt. Die read-only-Visualisierung FÜHRT die Pro-Allocation-Sicht (Topf-/Allocation-Sicht) — sie spiegelte sie früher nur: ein `write_off` hat den Übertrags-Topf entwertet und zählt als Verbrauch. Verteilt nur die SSoT-Summen FIFO auf die zwei Töpfe, keine eigene Cap-/Fenster-Mathematik. | **Bestätigt korrekt** |
| §45b Verbrauchs-Schnitt (`6hcfP7xVj5R3Pg6p`, E2) | `server/storage/budget/allocation-window.ts` (`countedConsumptionWhere`, Glied (a')) und `allocation-storage.ts` (`getExcluded45bConsumption`, `ersetztDurchStartwertIds`) | **Als Used gezählt — AUSSER auf einer vom Startwert ersetzten Allocation** | Korrekt. Ein vom Startwert ersetzter Übertrag ist abgegolten; seine Verfallsbuchung ist kein Verbrauch, sonst ginge dasselbe Guthaben zweimal ab (Stammticket, Ergänzung E2). Buchung und Storno darauf zählen nach dem Cutoff weiter (R4). | **Neu, 25.09.2026** |
